...
3. **If `result:drift`,** show the user the lists of entries to add and remove, for both the Skills group and the Scripts group, and ask for a one-line yes/no confirmation before applying.
...
The script prints a header, a Skills section, a Scripts section, and a final status line. Each section lists the entries already in sync, the entries to add, and the entries to remove; empty sub-lists are omitted:
...
* Collects desired script entries by recursively scanning each skill folder for files whose extension is in the runner map (`.mjs` -> `node`, `.zsh` -> `zsh`), skipping `node_modules`, `.git`, and dotfiles. Each match becomes `Bash(<runner> <repo-relative-path>:*)`.
* Buckets `permissions.allow` entries: `^Skill\(([^)]+)\)$` identifies the Skills group, and a `Bash(<runner> <path>:*)` entry whose path sits under the skills directory and whose extension matches the declared runner identifies the Scripts group. Entries in neither group (other `Bash(...)`, `Read(...)`, `WebSearch`, etc.) are always preserved untouched.

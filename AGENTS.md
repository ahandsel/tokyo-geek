...
* PWA icons in `contents/public/` are committed static files. The generator is no longer a dependency, so regenerate them on demand with `pnpm dlx @vite-pwa/assets-generator` when the source icon changes.

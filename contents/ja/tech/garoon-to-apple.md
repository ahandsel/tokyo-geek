...
> [!NOTE] 補足：URL フィールドに貼り付けたときにブラウザーが改行を取り除いても動作するように、コードではブロックコメントを使っています。
...
> [!NOTE] 補足：ガルーンのドキュメントでは、JavaScript API が終日のイベントの終了を最終日の 00:00:00 として返すことが示唆されている一方で、REST API は 23:59:59 を返しますが、複数日にわたる終日のイベントでどちらが返るかは未確認です。
...
`UID`が変わらなければ、イベントの日程変更後に再エクスポートしても既存のイベントが更新されます。
...
この記事やブックマークレットがお役に立ちましたら、[コーヒーをおごって][coffee]いただけると嬉しいです。
